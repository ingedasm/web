	...
	if (!gx.lang.emptyObject(vStruct) && !gx.lang.emptyObject(gx.rtPicture(vStruct, Ctrl)) && !gx.lang.emptyObject(vStruct.type)) {
		try {
			switch (vStruct.type) {
				case 'int':
				case 'decimal':
					return gx.num.formatNumber(value, vStruct.dec, gx.rtPicture(vStruct, Ctrl), vStruct.len, vStruct.sign, true, vStruct.emptyAsNull);
				case 'char':
					return value;
					//case 'date':
...
gx.dom_i =(function($){
	...
	isRoundedBtnWrapper: function( $el) {
		return $el.hasClass("BtnRight") || $el.hasClass("BtnLeft") || $el.hasClass("BtnBackground");
	},
		...
		try {
			...
			if (ssRules) {
				for (var i = 0, len = ssRules.length; i < len; i++) {
					...
					if (fixUrls && !cssText.includes('@import')) {
	...
	isTextControl: function (Ctrl) {
		return Ctrl && Ctrl.tagName === "INPUT" && Ctrl.type === "text" || Ctrl.tagName === "TEXTAREA";
	},
	...
	removeControlSafe: function (ctrl) {
		if (gx.dom.shouldPurge()) {
			...
			this.removeControl(this._avoidLeaksDiv);
		...
		if (useLeavingFx || useEnteringFx) {
			...
			if (oldEl.tagName != 'BODY' && newEl.tagName != 'BODY') {
				if (useLeavingFx) {
					...
					var cssRulesNoImports = [],
					cssRulesOnlyImports = cssRules.filter((el, index) => {
						if(el.includes('@import')){
							return el;
						} else {
							cssRulesNoImports.push(el);
						}
					});

					var insertAllRules = (ruleList) => {
						ruleList.forEach(rule => {
							try {
								styleSheet.insertRule(rule, styleSheet.cssRules.length);
							}
							catch (e) {
								gx.dbg.write('Warning: CSS Stylesheet rule not loaded:' + rule);									
							}
						})
					}
					insertAllRules(cssRulesOnlyImports);
					insertAllRules(cssRulesNoImports);
		...
		return {
			...
			html5: function (root) {
				var browser = gx.util.browser,
					isIE = browser.isIE(),
					i,
					styleBuffer = (gx.O.DSO) ? [`@layer ${DOM_FIXES_STYLE_ID} {`] : [],
					styleEl;
				...
				$tables.each(function () {
					...
					if (cellPadding !== undefined) {
						...
						$("tbody > tr > td, tbody > tr > th, tr > td, tr > th", this).each(function () {
							this.className = TABLE_PADDING_CLASS_PREFIX + cellPadding + " " + this.className.replace(TABLE_PADDING_CLASS_REGEX, "").trim();
						});
					}
					...
					if (cellSpacing !== undefined) {
						...
						this.className = TABLE_SPACING_CLASS_PREFIX + cellSpacing + " " + this.className.replace(TABLE_SPACING_CLASS_REGEX, "").trim();
					}
				});

				var fixCSSContent = `${Mustache.render(cellPaddingsTemplate, newCellPaddings)}${Mustache.render(cellSpacingsTemplate, newCellSpacings)}`;
				if (fixCSSContent.length > 0) {
					if (gx.O.DSO) {
						gx.dom.setStyleElement(styleEl, ` @layer ${DOM_FIXES_STYLE_ID} { ${fixCSSContent} }`, true);
					}
					else {
						gx.dom.setStyleElement(styleEl, fixCSSContent, true);					
					}
				}
			},
...
gx.evt_i = (function($) {
	...
	onfocus_impl: function (Ctrl, gxFocusCtrl, gxWCP, gxInMasterPage, gxCurrentRow, gxCurrentGrid, gxAddLines) {
		...
		try {
			...
			gx.evt.setReady(false, "onfocus_impl");
			if (!gx.fn.checkPopupFocus(Ctrl)) {
				gx.evt.setReady(true, "onfocus_impl");
				return onfocusDeferred.resolve(false).promise();
			}
			...
			gx.csv.checkRowChange(lastId, gridChange, gxO, lastRow).done(function (rowValidated) {
				...
				var afterValidateFn = function () {
					if (gx.csv.invalidControl == null && Ctrl === document.activeElement && 
						(Ctrl.nodeName === 'SELECT' || (gx.dom.isTextControl(Ctrl) && Ctrl.selectionStart === Ctrl.selectionEnd))) {
	...
	fireControlValueChange: function(gxO, Ctrl, evt) {
		...
		if (vStruct && vStruct.evt_cvc) {
			...
			if (vStruct.grid) {
				...
				gxCurrentRow = gx.fn.controlRowIndex(Ctrl);
			}
			gx.fn.disableKeys();
	...
	fireControlValueChanging: function(Ctrl) {
		var gxO = gx.fn.getControlContext(Ctrl) || gx.O;
		var deferred = $.Deferred();
		if (!gxO || !Ctrl) {
			//This should be an error..
			return deferred.resolve();
	...
	oncontrolvaluechanging: function (event) {
		...
		if (!Ctrl)
			return;
		gx.evt.fireControlValueChanging(Ctrl);
	},

	oncontrolpasting: function (event) {
		var Ctrl = gx.evt.source(event);
		if (!Ctrl)
			return;
		gx.lang.doCallTimeout( gx.evt.fireControlValueChanging, gx.evt, [Ctrl], 0);
	},
		...
		if (vStruct) {
			...
			if (type == 'int' || type == 'decimal') {
				...
				else
				{
					...
					if (/[^\+\-\.\,]/.test(event.key) && /\D/.test(event.key) && !event.ctrlKey && isNaN(Number(event.key))) {
						return false;
					} else {
						if (event.key === gx.thousandSeparator) {
							return true;
						}
						if (event.key === gx.decimalPoint) {
							let start = Ctrl.selectionStart
							let end = Ctrl.selectionEnd
							let selectedText = value.substring(start, end)
							if (selectedText.length > 0 && gx.text.contains(selectedText, event.key))
								return true
							return !gx.text.contains(value, event.key);
						}
						if (/[\+\-]/.test(event.key)) {
							return !gx.text.contains( value, event.key);
						}
	...
	onchange_impl: function(Ctrl, event, preventValid) {
		...
		gx.evt.setReady(false,"onchange_impl");
		...
		var vStruct = gxO.getValidStructFld(Ctrl);
		var changed = false;
		var focusControl = gx.O.focusControl;
		var finallyCallback = function() {
			...
			if (doValidControls && (typeof(Ctrl.GXFormatError) == 'undefined' || Ctrl.GXFormatError == false)) {//doValidControls solo si onblur valida ok
				gx.csv.validControls(focusControl, focusControl + 1, true, gxO).then(endCallback);
			}
			else {
				endCallback();
			}
		...
		if (gx.csv.controlValueHasChanged(Ctrl, CtrlValue, vStruct)) {
			...
			var	execOnChange = true;
			if (vStruct) {
				var sRow = vStruct.grid?gx.fn.controlRowIndex(Ctrl):undefined;
				if (typeof(vStruct.c2v) == 'function')
					vStruct.c2v(sRow);
				if (typeof(vStruct.v2bc) == 'function')
					vStruct.v2bc.call(gxO, sRow);
			}
			if (!preventValid) {
			...
			var validationCallback = function() {
				...
				if (Ctrl.type == "select-one" || Ctrl.type == "checkbox" || Ctrl.type == "radio") {
					execOnChange = false;
					gx.evt.setReady(false, "validationCallback");
					var ctrlRow = (vStruct) ? gx.fn.currentGridRowImpl(vStruct.grid) : null;
					gx.csv.validate(Ctrl, vStruct.id || focusControl, true, gxO, ctrlRow).done(function(isValid) {
						if (isValid) {
							if (gx.csv.invalidControl == Ctrl) {
								gx.csv.invalidControl = null;
							}
						...
						gx.evt.setReady(true,"validationCallback");
						deferred.resolve();
					}).fail(function() {
						gx.evt.setReady(true, "validationCallback");
						deferred.resolve();
	...
	setProcessing: function (opts) {
		var value = opts.status, 
			cond = opts.cond,
			ctx = opts.ctx;
		if ((typeof (cond) == 'undefined') || cond) {
			if (!gx.evt.redirecting) {
				...
				if (gx.dbg.enabled) {
					console.groupCollapsed("setProcessing ${ctx} ${gx.evt.processing}");
					console.trace(); 
					console.groupEnd();
				}
				gx.evt.setReady(!gx.evt.processing, "setProcessing");
	...
	doAfterProcessing: function(fnc, ctx) {
		if (gx.evt.processing) {
			gx.fx.obs.addObserver('gx.endprocessing', ctx, fnc, { single: true });
		} 
		else {
			fnc.call(ctx);
		}
	...
	userReadyThresholdWait: function () {
		if (!gx.evt.userReadyTStamp) {
			gx.evt.userReadyTStamp = Date.now();
		}
		return (Date.now() > gx.evt.userReadyTStamp + gx.evt.userReadyThreshold)		
	},

	setReady: function (isReady, Context) {
		gx.evt.userReadyCnt += isReady ? -1 : 1;
		gx.evt.userReadyCnt = gx.evt.userReadyCnt < 0 ? 0 : gx.evt.userReadyCnt;
		if (gx.evt.userReadyCnt == 0) {
			gx.evt._userReady = true;
			gx.evt.userReadyTStamp = Date.now();
		}
		else {
		...
		if (gx.dbg.enabled) {
			console.groupCollapsed("setReady ${Context} ${gx.evt.userReadyCnt}");
			console.trace(); 
			console.groupEnd();
		}
	},
	...
	canExecuteEvent: function(evt, force) {
		return !(!force && ((gx.spa.isNavigating() || gx.evt.redirecting) || !gx.csv.isvalidating() && (gx.lang.emptyObject(evt) || gx.evt.processing ))); 
	},
	...
	execEvt: function ( cmpCtx, inMaster, evt, ctrl, gridId, sync, srvCommand, disableForm, callback, force, failCallback, enterKeyPressed) {
		...
		var notifyValidation = function () {
			gx.fx.obs.notify('gx.validation', null, (function () {
				//Input Format errors must be checked after validation.
				...
				if (!srvCommand && (gx.grid.drawAtServer || (ctrl && ctrl.nodeName == 'INPUT' && ctrl.type == 'submit'))) {
					...
					var oldDoPost = function() {
						gx.ajax.doPost((gx.http.useNamedParameters(gx.ajax.selfUrl())? 'gxevent=' : '') + gx.ajax.encryptParms(gx.pO, 'gxajaxEvt'), sync, disableForm, gridId, rowId, callback);
						fnc_always();
					};
	...
	execEvtSubmit: function (evt, ctrl) {
		if (!gx.isInputEnabled())
			return;
		gx.evt.setProcessing({status:true, ctx:"execEvtSubmit"});
		gx.O.startFeedback();
	...
	execCliEvt: function ( cmpCtx, inMaster, cliEvtName, evtGridName, gridRow, parms) {
		...
		if (gxObj != null) {
			...
			gx.evt.setProcessing({status:true, cond:!isServerEvent, ctx:"execCliEvt"});
			gxObj.execC2VFunctions();
			var callback = function () {
				gx.popup.waitCallback(function () {
					gx.evt.setProcessing({status:false, cond:!isServerEvent, ctx:"execCliEvt"});
				});
	...
	return {
				...
				try {
					...
					if (TargetControl > FromControl) {
						...
						if (controlsToValidate.length > 0) {
							gx.evt.setProcessing({status:true, ctx:"validControls"});
							runControlValidation(controlsToValidate, this, function () {
								...
								gx.evt.setProcessing({status:false, ctx:"validControls"});
								resolve(bRet);
							});
							return deferred.promise();
						}
						...
						var vStructValidCallback = function (validRet) {
							...
							if (validStruct.isvalid != null && ctrlIsAccepted && Ctrl.getAttribute(gx.csv.GX_VALID_ATTRIBUTE) != "1" && !toUpperRowInGrid) {
								...
								if (gx.pO.supportAjaxEvents) {
									...
									if (evtName) {
										gx.evt.dispatcher.dispatch(gxO.getServerEventName(evtName), gxO, gridId, ctrlRow, serverEvent)
											.done(callback)
											.fail(callback);
									}
									else {
										gxO[evtName](ctrlRow).then(callback);
									}
									return;
								}
								else {
									...
									gxO[evtName](ctrlRow).then(function () {
			...
			invalidateCtrl: function (Ctrl, keyEvt) {
				try {
					...
					if (validStruct && validStruct.grid != 0 && (keyEvt === true || gx.evt.keyModifiesValue(keyEvt))) {
			...
			refreshControls: function (validStruct, gxO, row) {
				...
				for (var i = 0; i < len; i++) {
					try {
						...
						{
							var curRow = gx.fn.currentGridRowImpl( VStr.grid);
							if (gx.text.endsWith(row, curRow)) {	
								gx.fn.v2c(VStr);
			...
			setValidValues: function (InputArr, OutputArr, ValuesArr) {
				...
				gx.O.refreshOlds();
				gx.fn.enableDisableDelete(gx.O);
				if (anyGrid) {
...
gx.http_i = (function ($) {
		...
		return {
			...
			setCookie: function (name, value, nDays, isSession, path) {
				...
				cookie.push(isSession ? "session": "expires=" + exp.toGMTString());
			...
			onRequestError: function (req, info) {
				...
				gx.evt.setProcessing({status:false, cond:!gx.csv.isvalidating(), ctx:"onRequestError"});
				if (req.readyState == this.STATE_DONE) {
			...
			iframeOnload: function () {
				...
				if (iFrame) {
					try {
						...
						var internalErrorMessage = "Unknown error";
					 ...
					 if (event.status < 200 || event.status > 299) {
						 ...
						 if (gx.http.validJsonResponse( event, data)) {
							...
							if (DataObj == null)
								...
								DataObj.rowId = this.rowId;
								DataObj.gridId = this.gridId;
			...
			loadStyle: function (url, callback, beforeTheme, id, layer) {
				var style;
				if (layer) {
					style = document.querySelector(`style[data-gx-href="${id}"]`);
				} else {
					style = id ? gx.dom.byId(id) : null;					
				}				
				...
				if (style) {
					...
					if (layer) {
						...
						style.setAttribute( "data-gx-href", url);
						}
						style.innerHTML = `@import url("${url}") layer(${layer})`;
					}
					else {
			...
			layerNaming: function( styleName) {
				return styleName.replace(/\?\w*$/,'').replace(/[\/\.]/g,'_');
			},
				...
				if (!gx.lang.emptyObject(styles)) {
					for (var i = 0; i < styles.length; i++) {
						...
						if (!gx.cache.fileLoaded(style)) {
							...
							this.loadStyle(style, undefined, true, undefined, this.layerNaming(style));
						}
					}
				}
			},
...
(function($) {
			...
			if (bcProp instanceof Array) {
				...
				var v2bcFunc = function (rowId) {
					...
					if (vStruct.type == 'date') {
						...
						var ctrl = gx.fn.getControlGridRef(vStruct.fld, vStruct.grid, rowId);
						if (ctrl && $(ctrl).val())
							propValue = "gxformdataref:" + ctrl.id;
					}
					this.valueToBCProp(this[bcVarName], bcProp, vStruct, propValue, rowId);
				};
				for (var i = 0; i < len; i++) {
	...
	prot.valueToBCProp = function (bc, bcProp, vStruct, pValue, pRow) {
		try {
			if(bc){
				var obj = bc,
					len = bcProp.length,
					...
					gridlvl = 0,
					sRow;
				var grid2obj = function(obj) {
					if (gx.lang.isArray(obj) && vStruct && vStruct.grid) {
						var gxGrid = gx.fn.getGridObj(vStruct.grid, lvlRow && lvlRow.length === 4 ? lvlRow : undefined);
						var firstRecordOnPage = (gxGrid && gxGrid.grid && !gxGrid.InfiniteScrolling) ? gxGrid.grid.firstRecordOnPage : 0;
						sRow = (lvlRow) ? lvlRow : gx.fn.currentGridRow(vStruct.grid);
						if (sRow.length > 3) {
							gridlvl += 1;
							sRow = sRow.substr(-4 * gridlvl, 4);
				...
				if (obj) {
					pty = bcProp.length - 1;
					obj[bcProp[pty]] = pValue;
					if (obj[bcProp[pty]  + "_N"] !== undefined) {
						obj[bcProp[pty]  + "_N"] = 0;
					}
				}
			}
	...
	prot.execV2CFunctions = function (Show, userControls, ShowGridUcs, ConditionalFnc) {
		...
		$.each(userControls, function (i, uc) {
			if (uc && gxO == uc.ParentObject && (ShowGridUcs || !uc.GridId || uc.gxGridName) && (!ConditionalFnc || ConditionalFnc(uc))) {
	...
	prot.gxOWithEnter = function (ctrlEvt) {
		if (ctrlEvt) {
			var gxO = gx.fn.getControlContext(ctrlEvt);
	...
	var evaluateCode = function (conditionCode) {
		...
		var fn;
		try {
			fn = new Function( gx.text.contains(conditionCode.toLowerCase(), "return") ? conditionCode : "return " + conditionCode);
		}
		catch (e) {
	...
	var dispatchControlEvent = function (domEvt) {
		...
		if (domEvt.type === "keypress" && (gx.evt.processing || gx.evt.redirecting)) {
		...
		if (gx.dom.isRoundedBtnWrapper($target)) {
			$target = $target.find('input');
			$currentTarget = $currentTarget.find('input');			
		}			
		var gxO = gx.fn.getControlContext($currentTarget);
		...
		var fnc = function() {
			var fld = $currentTarget.attr(gx.GxObject.GX_EVENT_CONTROL_DATA_ATTR);
	...
	prot.handleControlEvent = function (domEvt, fld, el) {
		...
		if (gx.util.inArray(vStruct.ctrltype, gx.GxObject.GX_EVENT_EXCLUDED_CTRLTYPES))
			return;
	...
	prot.registerCtrlsEventHandlers = function () {
		...
		for (i=0, len=ctrlIds.length; i<len; i++) {
			...
			needsDataContext = (this.hasEnterEvent && !gx.lang.emptyObject(vStruct.ctrltype)) ||
								vStruct.evt_cvc || vStruct.evt_cvcing;
			if (hasEvt || needsDataContext) {
				if (hasEvt) {
		...
		if (isParentObject) {
			if (!this.listeningControlEvents) {
				...
				$(eventSelector).each( function() {
					var $el = $(this).parent().first();
					while (gx.dom.isRoundedBtnWrapper($el)) {
						$el.on('click keypress', null, dispatchControlEvent);
						$el = $el.parent().first();						
					}
				});
			}
	...
	prot.onload = function (loadGrids, forceAfterLoad) {
		...
		$.when.apply($, arrDeferreds).done(function () {
			_this.execV2CFunctions(true, undefined, false, function(uc) {return uc.ownerGrid && uc.ownerGrid.parentGrid});
...
gx.webSocket = function (wsOpts) {
	...
	this.retryCount = 0;
	this.maxRetries = wsOpts.maxRetries || 6;
	this.retryDelay = wsOpts.retryDelay || 2500;
		...
		try {
			...
			this.ws.onclose = function () {
				...
				if (_this.opened) {
					...
					gx.fx.obs.notify('gx.ws.OnClose'+_this.namespace, []);
				}
			};
		} 
		catch(e) {
	...
	this.close = function() {
		...
		_this.stopRetry = true;
		if (_this.retryHandle) {
			clearInterval(_this.retryHandle.t);
		}
	};
...
gx.popup = (function ($) {
	...
	return {
		...
		assignPromptFieldValue: function (obj, gxO) {
		...
		assignAndValidatePromptField: function (obj, gxO) {
			...
			var callback = (function () {
				gx.popup.assignPromptFieldValue(obj, gxO);
...
gx.ajax = (function ($) {
	...
	return {
		...
		message: function (method, gxO, grid, row, serverSide, disableForm) {
			...
			return {
					...
					if (grid) {
						...
						var setGridsWithParents = function(grid, grids) {
							if (grid) {
								...
								if (curRow)	{
									grids[grid.gridName].lastRow = Math.max( 0, Number(curRow.substr( -4,4)) - 1);
								}
						...
						else if (this.gxO.isTransaction() && this.grid > 0 && this.row && parm.av === 'Gx_mode' && parm.fld === 'vMODE')
						{
							...
							if (parm.prop) {
								if (parm.prop == 'GridRC') {
									...
									value = {gridRC: gx.fn.getHidden( this.gxO.sPrefix() + parm.av +rowSuffix), rowSuffix:rowSuffix, grid:parm.grid};
								}
								else {
									if (parm.prop == 'GridCurrRow') {
										value = gx.fn.currentGridRowImpl(parm.grid);
									}
									else {
				...
				getParmVarVStruct: function (parm) {
					...
					if (parm.fld !== undefined) {
						...
						if (this.grid && this.row) {
							gridObject = this.gxO.getGridById(this.grid, this.row);
							if (gridObject) {
				...
				doAjaxCall: function (Synch, disableForm) {
					...
					gx.evt.setProcessing({status:true, ctx:"doAjaxCall"});
				...
				afterAjaxCallHandler: function (callFailed, DataObj, gxO) {
					gxO = gxO || gx.O;
					gx.evt.setProcessing({status:false, ctx:"afterAjaxCallHandler"});
				...
				ajaxCallHandler: function (type, data, event) {
					...
					if (event.status == gx.http.STATUS_FORBIDDEN) {
						gx.util.alert.showError(gx.getMessage("GXM_runtimeappsrv") + ' (' + event.status + ' ' + event.statusText + ')');
						callFailed = true;
					}
					else {
					...
					this.afterAjaxCallHandler(callFailed, DataObj, this.gxO);
				},
		...
		saveJsonResponse: function (jsonResponseString) {
			this.lastJsonResponse = JSON.parse(gx.lang.htmlDecode(jsonResponseString));
		},
			...
			if (gx.evt.srvCommand) {
				...
				if (redirectCommand && redirectCommand.redirect.forceDisableFrm === 1) {
					...
					doFunc(fn.setJsonComponents, response.gxComponents, response.DynComponentMap, response.gxHiddens, function (newComponents) {
						...
						fn.enableDisableDelete(gxO);
						
						if (isPostback) {
							retloadJsonGridsDeferred = doFunc(fn.loadJsonGrids, response.gxGrids, isPostback, firstLevelGridUCs);
						}
						...
						var fnc = function(gridUCs) {	
								gridUCs	= gridUCs || [];
								var userControls;
								if (gx.pO.supportAjaxEvents) {
									userControls = gx.lang.arrayUnique(propUCs.concat(valuesUCs, newCompUCs, gridUCs, hiddensUCs), function (UC, UC2) { return UC.ControlName === UC2.ControlName});
								}
						...
						var end = function () {
							if (context.fullAjaxCall && !context.autoRefreshing) {
						...
						var afterEndDeferred = [];
						if (isPostback) {
							gxO.execV2CFunctions(true, firstLevelGridUCs, false);							
							if (retloadJsonGridsDeferred) {								
								afterEndDeferred.push(retloadJsonGridsDeferred.done(fnc));
							}
							if (newComponents && newComponents.length) {
								afterEndDeferred.push(fnc.apply(this, []));
							}
						}
						$.when.apply($, afterEndDeferred).done(end);						
					}, isPostback);
				}
			}
			return deferred.promise();
		...
		isFormEnabled: function () {
			var context = gx.getActiveContext();
			if (context == top && gx.popup.getPopup() != null) {
				return true;
			}
			if (gx.pO)
				return !context.gx.pO.isFeedbackOn() && !context.gx.O.isFeedbackOn();
			return true;
		},
		...
		newRows: function (CmpCtx, InMasterPage, GridName, gxRows, gxRowIndex, gxRowId, gxParentRow, gHandler) {
			...
			parameters = parameters.concat(gridObj.getParmsValues(false, gridObj.postingVariables, false));
		...
		parmToUrl: function (Parm, serializeControls) {
			...
			if (serializeControls) {
				var ctrlJSON = gx.ajax.ControlValuesObject(Parm, value);
				if (ctrlJSON) {
					value = ctrlJSON;
				}
			}
...
gx.util.balloon = {
	...
	impl: function () {
		...
		this.create = function (id, Control, Class, Msg) {
			...
			div.classList.add("gx-error__message-container");
			...
			try {
				...
				gx.lang.requestAnimationFrame(function () {
					...
					if (gx.csv.overlap === true) {
						...
						$div.fadeIn({queue:false}).css("display","block");
						$div.position({
							my: my,
							at: at,
							of: ofEl,
